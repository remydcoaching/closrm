...
## 4. Migration Branding — T-020 (couleur d'accent + logo workspace)

Ajoute les colonnes de personnalisation au workspace et crée le bucket Storage pour les logos.
...
SELECT column_name FROM information_schema.columns
WHERE table_name = 'workspaces' AND column_name IN ('accent_color', 'logo_url');

-- Vérifier le bucket
SELECT id FROM storage.buckets WHERE id = 'workspace-logos';
```
...
Crée la table `planning_templates` pour stocker les gabarits de semaine type (blocs journaliers réutilisables).
...
```sql
-- Vérifier que la table planning_templates existe avec les bonnes colonnes
select column_name, data_type
from information_schema.columns
where table_name = 'planning_templates'
order by ordinal_position;
```

Tu dois voir ces colonnes :
- `id` ← uuid, clé primaire
- `workspace_id` ← uuid, FK vers workspaces
- `name` ← text
- `description` ← text (nullable)
- `blocks` ← jsonb (tableau de blocs)
