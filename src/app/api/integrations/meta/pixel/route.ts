...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const body = (await req.json().catch(() => null)) as { pixel_id?: string | null } | null
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Erreur lors de la sauvegarde du Pixel ID.' }, { status: 500 })
    }

    return NextResponse.json({ data: { pixel_id: pixelId } })
  } catch (err) {
