...
interface ClosingClientProps {
  ...
  initialCounts: { today: number; upcoming: number; overdue: number; done: number; cancelled: number }
}

export default function ClosingClient({ initialCalls, initialMeta, initialCounts }: ClosingClientProps) {
  const [calls, setCalls] = useState<CallWithLead[]>(initialCalls)
