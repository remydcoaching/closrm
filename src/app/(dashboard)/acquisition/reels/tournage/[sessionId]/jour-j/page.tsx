...
export default async function SessionJourJRedirectPage({
  params,
}: {
  ...
  if (ids.length === 0) {
    redirect('/acquisition/reels/tournage/jour-j?session=' + sessionId)
  }
  redirect(`/acquisition/reels/tournage/jour-j?reel=${ids.join(',')}&session=${sessionId}`)
}
