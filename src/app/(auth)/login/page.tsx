...
export default function LoginPage() {
  ...
  const lbl: React.CSSProperties = { display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--text-label)', marginBottom: 8, letterSpacing: '0.15em', textTransform: 'uppercase' }
  const ico: React.CSSProperties = { position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-label)' }
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, rgba(17,17,19,0.9), rgba(12,12,14,0.95))', border: '1px solid var(--border-primary)', borderRadius: 16, padding: '36px 32px', boxShadow: '0 0 60px rgba(0,200,83,0.04)' }}>
      <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>Bon retour 👋</h2>
      <p style={{ fontSize: 13, color: 'var(--text-tertiary)', marginBottom: 28 }}>Connectez-vous à votre espace ClosRM</p>
      ...
      <form onSubmit={submit}>
        ...
        <div style={{ textAlign: 'right', marginBottom: 20 }}>
          <Link href="/reset-password" style={{ fontSize: 12, color: 'var(--text-tertiary)', textDecoration: 'none' }}>Mot de passe oublié ?</Link>
        </div>
      ...
      <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid var(--border-primary)', textAlign: 'center' }}>
        <p style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>Pas encore de compte ? <Link href="/register" style={{ color: 'var(--color-primary)', fontWeight: 600, textDecoration: 'none' }}>S&apos;inscrire</Link></p>
      </div>
    </div>
  )
}
