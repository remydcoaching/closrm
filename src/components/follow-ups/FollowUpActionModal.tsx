...
export default function FollowUpActionModal({ followUp, onClose, onAction }: Props) {
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose() }} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#0f0f11', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 440 }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#666', marginTop: 4 }}>{followUp.lead.first_name} {followUp.lead.last_name}</p>
          </div>
        ...
        {mode === 'menu' ? (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase' as const, marginBottom: 8, paddingLeft: 4 }}>Prospect intéressé</div>
              <ActionButton icon={Phone} label="Planifier un RDV" desc="Le prospect est chaud → planifier un appel" color="#00C853" onClick={() => { onAction({ type: 'schedule_call', leadId: followUp.lead.id }); onClose() }} />
            </div>

            {/* Relancer */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase' as const, marginBottom: 8, paddingLeft: 4 }}>Relancer plus tard</div>
            ...
            {/* Abandonner */}
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase' as const, marginBottom: 8, paddingLeft: 4 }}>Abandonner</div>
          ...
          <div>
            {/* Presets */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase' as const, marginBottom: 8 }}>Relancer dans</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {presets.map((p) => (
                  <button key={p.d} onClick={() => { setDays(p.d); setUseCustomDate(false) }} style={{
                    ...
                    border: !useCustomDate && days === p.d ? '2px solid #3b82f6' : '1px solid rgba(255,255,255,0.06)',
                    background: !useCustomDate && days === p.d ? 'rgba(59,130,246,0.08)' : 'transparent',
                    color: !useCustomDate && days === p.d ? '#3b82f6' : '#888',
                  }}>
                    {p.label}
                  </button>
                ))}
            ...
            {/* Custom date */}
            <div style={{ marginBottom: 16 }}>
              <button onClick={() => setUseCustomDate(!useCustomDate)} style={{ fontSize: 12, color: '#888', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 8 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase' as const, marginBottom: 8 }}>Raison (optionnel)</div>
              <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Ex: Envoi vidéo témoignage" style={inputS} />
            </div>

            {/* Actions */}
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setMode('menu')} style={{ flex: 1, padding: '10px 0', borderRadius: 10, fontSize: 13, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: '#888', cursor: 'pointer' }}>Retour</button>
              <button onClick={handleReschedule} style={{ flex: 1, padding: '10px 0', borderRadius: 10, fontSize: 13, fontWeight: 600, background: '#00C853', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
...
function ActionButton({ icon: Icon, label, desc, color, onClick }: { icon: typeof Phone; label: string; desc: string; color: string; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{
      ...
      background: hovered ? 'rgba(255,255,255,0.04)' : 'transparent', transition: 'background 0.15s', marginBottom: 4,
    }}>
