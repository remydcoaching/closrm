...
export default function ContactPanel({ conversation }: Props) {
  ...
  return (
    <aside
      style={{ width: 280 }}
      className="flex flex-col border-l border-[#1a1a1a] bg-[#0d0d0d] overflow-y-auto shrink-0"
    >
      {/* Header */}
      <div className="flex flex-col items-center py-6 px-5 border-b border-[#151515]">
        <div
        ...
        {handle && (
          <p className="text-[11px] text-[#555] mt-0.5">{handle}</p>
        )}
      </div>
      ...
      {!conversation.lead_id ? (
        ...
        <div className="px-5 py-[14px]">
          <p className="text-[11px] text-[#444] italic">Aucun lead associé</p>
        </div>
      ) : loadingLead ? (
        /* Loading skeleton */
        <div className="px-5 py-[14px] space-y-3">
          <div className="h-3 w-24 bg-[#161616] rounded animate-pulse" />
          <div className="h-5 w-32 bg-[#161616] rounded animate-pulse" />
          <div className="h-3 w-20 bg-[#161616] rounded animate-pulse mt-4" />
          <div className="flex gap-1">
            <div className="h-5 w-14 bg-[#161616] rounded-full animate-pulse" />
            <div className="h-5 w-10 bg-[#161616] rounded-full animate-pulse" />
          </div>
        </div>
      ) : lead ? (
        <>
          ...
          <div className="px-5 py-[14px] border-b border-[#131313]">
            <p className="text-[9px] uppercase text-[#444] tracking-[0.8px] font-semibold mb-2">
              Statut pipeline
            </p>
            {(() => {
          ...
          <div className="px-5 py-[14px] border-b border-[#131313]">
            ...
            {lead.tags.length === 0 ? (
              <p className="text-[11px] text-[#444] italic">Aucun tag</p>
            ) : (
              <div className="flex flex-wrap gap-[5px]">
                {lead.tags.map(tag => (
                  <span
                    key={tag}
                    className="px-[10px] py-[3px] bg-[#161616] border border-[#1f1f1f] rounded-full text-[10px] text-[#888]"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div className="px-5 py-[14px] border-b border-[#131313]">
            ...
            {nextCall ? (
              <div className="bg-[#141414] border border-[#1f1f1f] rounded-[10px] p-3 flex gap-2.5 items-start">
                <svg className="w-3.5 h-3.5 text-[#E53E3E] mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <p className="text-[11px] text-[#444] italic">Aucun planifié</p>
            )}
          </div>
          ...
          <div className="px-5 py-[14px] border-b border-[#131313]">
            <Link
              href={`/leads/${lead.id}`}
              className="flex items-center gap-1.5 text-[12px] text-[#E53E3E] hover:text-[#C53030] transition-colors font-medium"
            >
          ...
          <div className="px-5 py-[14px] flex-1">
            <p className="text-[9px] uppercase text-[#444] tracking-[0.8px] font-semibold mb-2 flex items-center gap-1.5">
              Notes
              {savingNotes && (
                <span className="text-[8px] text-[#555] normal-case tracking-normal">Sauvegarde…</span>
              )}
            </p>
            <textarea
              ref={notesRef}
              ...
              rows={5}
              className="w-full bg-[#111] border border-[#1f1f1f] rounded-lg p-2.5 text-[12px] text-[#ccc] placeholder-[#444] resize-none focus:outline-none focus:border-[#2a2a2a] transition-colors"
            />
          </div>
        </>
      ) : (
        /* Lead fetch failed */
        <div className="px-5 py-[14px]">
          <p className="text-[11px] text-[#444] italic">Impossible de charger le lead</p>
        </div>
      )}
    </aside>
  )
