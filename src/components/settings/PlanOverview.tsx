...
export default function PlanOverview() {
  ...
  if (loading) {
    return <p style={{ color: 'var(--text-tertiary)', fontSize: 13 }}>Chargement…</p>
  }

  if (!data || !data.plan) {
    return <p style={{ color: 'var(--text-tertiary)', fontSize: 13 }}>Aucune donnée de plan disponible.</p>
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ borderTop: '1px solid var(--border-primary)', paddingTop: 16 }}>
        <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: 0.4 }}>
...
function MetricCard({ label, value, subtle }: { label: string; value: string; subtle?: string }) {
  return (
    ...
    >
      ...
      {subtle && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{subtle}</div>}
    </div>
  )
}

const RESOURCE_META: Record<string, { label: string; icon: React.ReactNode }> = {
  email: { label: 'Emails', icon: <Mail size={16} /> },
  ai_tokens: { label: 'Tokens IA', icon: <Bot size={16} /> },
  whatsapp: { label: 'WhatsApp', icon: <MessageCircle size={16} /> },
  sms: { label: 'SMS', icon: <Zap size={16} /> },
}
