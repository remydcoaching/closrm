...
const sectionTitle = { fontSize: 12, fontWeight: 700, color: '#555', textTransform: 'uppercase' as const, letterSpacing: '0.12em', marginBottom: 14 }
const inputStyle = {
    ...
    <div>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Téléphone</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#aaa', fontSize: 13 }}>
              <Phone size={13} color="#555" />
              {lead.phone || <span style={{ color: '#444' }}>Non renseigné</span>}
            </div>
          </div>
          <div>
            <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Email</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#aaa', fontSize: 13 }}>
              <Mail size={13} color="#555" />
              {lead.email || <span style={{ color: '#444' }}>Non renseigné</span>}
            </div>
          </div>
          <div>
            <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Tentatives d&apos;appel</label>
            <div style={{ fontSize: 13, color: '#aaa' }}>{lead.call_attempts} tentative{lead.call_attempts > 1 ? 's' : ''}</div>
          </div>
          <div>
            <label style={{ fontSize: 11, color: '#555', marginBottom: 4, display: 'block' }}>Joint</label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}>
              {lead.reached
                ? <><CheckCircle size={13} color="#00C853" /><span style={{ color: '#00C853' }}>Oui</span></>
                : <><XCircle size={13} color="#555" /><span style={{ color: '#555' }}>Non</span></>
              }
            </div>
          </div>
        </div>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {tags.map(tag => (
            <span key={tag} style={{
              ...
              background: 'rgba(0,200,83,0.08)', color: '#00C853', border: '1px solid rgba(0,200,83,0.15)',
            }}>
              {tag}
              <button onClick={() => removeTag(tag)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#00C853', padding: 0 }}>
                <X size={10} />
              </button>
            </span>
          ))}
          {tags.length === 0 && <span style={{ fontSize: 12, color: '#444' }}>Aucun tag</span>}
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button onClick={addTag} style={{
            ...
            border: '1px solid rgba(255,255,255,0.06)', borderRadius: 7, color: '#888', cursor: 'pointer',
          }}>
            <Plus size={13} />
          </button>
        </div>
      ...
      <div style={card}>
        ...
        <p style={{ fontSize: 10, color: '#444', marginTop: 6 }}>Sauvegarde automatique</p>
      </div>
      ...
      <div style={card}>
        ...
        {timeline.length === 0 ? (
          <p style={{ fontSize: 13, color: '#444', textAlign: 'center', padding: '16px 0' }}>
                  ...
                  {item.type === 'call' ? (
                    <>
                      ...
                      {(item.data as Call).notes && (
                        <div style={{ fontSize: 12, color: '#666', marginTop: 3 }}>{(item.data as Call).notes}</div>
                      )}
                    </>
                  ) : (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>{(item.data as FollowUp).reason}</div>
                    </>
                  )}
