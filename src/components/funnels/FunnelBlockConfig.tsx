...
import type { FunnelBlock, FunnelBlockConfig as FunnelBlockConfigType } from '@/types'
...
export default function FunnelBlockConfig({ block, onChange }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {block.type === 'hero' && (
        <HeroConfig config={block.config as Parameters<typeof HeroConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'video' && (
      ...
      {block.type === 'form' && (
        <FormConfig config={block.config as Parameters<typeof FormConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'booking' && (
        <BookingConfig config={block.config as Parameters<typeof BookingConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'pricing' && (
        <PricingConfig config={block.config as Parameters<typeof PricingConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'faq' && (
      ...
      {block.type === 'cta' && (
        <CtaConfig config={block.config as Parameters<typeof CtaConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'text' && (
        <TextConfig config={block.config as Parameters<typeof TextConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'image' && (
        <ImageConfig config={block.config as Parameters<typeof ImageConfig>[0]['config']} onChange={c => handleConfigChange(c)} />
      )}
      {block.type === 'spacer' && (
