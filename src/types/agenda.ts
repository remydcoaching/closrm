...
export function bookingToAgendaEvent(b: BookingWithCalendar): AgendaBookingEvent {
  const leadName = b.lead
    ...
    : null
  // Override couleur explicite (`bookings.color`) > couleur héritée d'un
  // import de template (`form_data.color`) > couleur du calendrier > fallback.
  const personalColor =
  ...
  const resolvedColor =
    b.color
    ?? (b.is_personal ? personalColor : b.booking_calendar?.color ?? '#3b82f6')
  return {
    id: `booking-${b.id}`,
    kind: 'booking',
